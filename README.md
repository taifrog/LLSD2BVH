...
1. [GitHub Releases](https://github.com/taifrog/LLSD2BVH/releases) から `LLSD2BVH_v0.4.0.zip` をダウンロード
...
| 部位 | 説明 |
|------|------|
| ① 入力リスト | LLSD XML の一覧。ドラッグ＆ドロップ、または「追加…」で最大20件まで追加。選択して「削除」「↑」「↓」「クリア」「コピー」で編集。リストはファイル管理用、出力順はタイムラインの時刻順になります |
| ② タイムライン | 横方向の時間軸（0〜アニメーション時間）。各ポーズをブロックで表示、ドラッグで時刻を移動、ダブルクリックで数値入力。先頭ブロックは0s、末尾は duration に固定（濃色で表示）。2件以上で有効、1件時は無効。上部「－」「＋」「100%」「ズーム: n%」で100〜400%に拡大（±50%刻み、ホイールで拡大縮小、空所を左ドラッグでパン） |
...
| ④ 算出 Frame Time / 総フレーム | タイムラインから自動算出された Frame Time と総フレーム数を表示（例: `算出 Frame Time: 0.3125 総フレーム: 18 (Tpose+17 P1@0s) (+9補間)`）。非均一時に補間数が括弧で表示されます |
...
| ⑦ Units / SL互換 | 出力単位とSL互換。`自動`（推奨）は位置あり→`inch`＋`2フレーム`（先頭に基準フレームを複製）、位置なし→`meter`＋`1フレーム`。Frame Timeは現行では非表示・自動算出です |
| ⑧ 手を除外 | 含めるボーンの切替。既定は手を含まずチェックON（顔・尻尾は常時除外、手のみ切替可） |
| ⑨ Progress/Log | 変換進捗とログ表示 |
| ⑩ プレビュー / ビューア / 変換 / 閉じる | 「プレビュー」…出力BVHを3D棒人間で表示（変換後に有効）。「ビューア」…任意のBVHを3Dで表示（変換不要、常時有効、空で開いてビューア側の「参照…」やDnDで選択）。「変換」で実行、完了時にダイアログで出力パスとフレーム数を表示 |
...
| 部位 | 説明 |
|------|------|
| ① BVH選択 | 上部「BVH: … 参照…」で行が表示され、メイン画面の「プレビュー/ビューア」または単体起動 `python -m llsd2bvh.viewer` / `bvh-viewer` から開く。ドラッグ＆ドロップでも読込可 |
| ② 3Dビュー | 棒人間（骨線・関節点）と床グリッド。左ドラッグで回転、右ドラッグでパン、ホイールでズーム。左上 `yaw/pitch/dist`、右下に操作ヒントを表示 |
| ③ 情報行 | ファイル名・`Frames / FrameTime / Channels / Joints` と `f / t / dt`。補間ON時は `1->2 (50%) f=1.50 [補間]` のようにフレーム間を30fpsで連続表示 |
| ④ スライダー | `0〜Frames-1` をドラッグまたは `◀ / ▶` で1コマ移動。再生中は整数部のみ同期 |
| ⑤ 再生コントロール | `◀` `▶再生/■停止` `ループ` `▶` `速度(0.25x〜2x)` `補間`（既定ONで30fps滑らかにチャネルlerp＋角度最短wrap、OFFでコマ送り） `Tpose先頭をスキップ` `回転(SL互換/BVH標準)` |
| ⑥ 視点プリセット | `正面(180°)/側面(90°)/上面(85°) / リセット(20°/-12°)`。任意回転後はリセットで初期視点に戻す |
...
3. タイムラインでポーズのタイミングを調整し、アニメーション時間を確認（算出 Frame Time と総フレーム数が表示されます）。「－/＋」やホイールで拡大して細かく編集できます
4. 必要に応じて出力先や Units/SL互換を変更（通常は `自動` のままでOK）
5. 「変換」をクリック → ログに `書き出し完了` と表示されれば成功。出力された `.bvh` を Viewer や Blender で読み込んでください
6. 変換後は `プレビュー`、任意BVHの確認だけなら `ビューア` で3D棒人間表示（補間ONで滑らかに30fps再生、視点は正面/側面/上面/リセットで切替）
...
#       dist/LLSD2BVH_v0.4.0.zip (約44MB)
